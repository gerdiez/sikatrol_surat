<div class="p-5 rounded-lg bg-white">
    <div class="flex @can('tata usaha') justify-between @else justify-end @endcan items-center">
        @can('tata usaha')
            <a href="{{ route('surat.perintah.create') }}"
            ...
            <tbody>
                ...
                @else
                    @foreach ($surats as $surat)
                        @if ($surat->dibaca == 'true')
                            <tr
                                class="bg-gray-50 border-b hover:shadow-lg hover:border-b-4 hover:border-t-2 text-center">
                            @else
                        ...
                        <td class="py-4">
                            <div class="tooltip static z-20 hover:relative" data-tip="Lihat">
                                @can('unit kerja')
                                    <a wire:click.prevent="read({{ $surat->id }})"
                                ...
                                @endcan
                                @cannot('unit kerja')
                                    <a href={{ route('surat.perintah.show', $surat->id) }}
                                        class="mr-1 text-blue-600 hover:bg-blue-300 hover:rounded-md">
                                        <i class="fa-solid fa-eye rounded-md border border-blue-400 p-1"></i>
                                    </a>
                                @endcannot
                            ...
                            @can('tata usaha')
                                <div class="tooltip static z-20 hover:relative" data-tip="Ubah">
                                    <a href={{ route('surat.perintah.edit', $surat->id) }}
                                        ...
                                        <div class="relative w-full h-full max-w-md md:h-auto">
                                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                ...
                                                <div class="p-6 text-center">
                                                    <form action="{{ route('surat.perintah.destroy', $surat->id) }}"
                                                        method="post">
                                                        @method('delete')
                                                        @csrf
